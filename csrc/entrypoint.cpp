...
aclError aclrtMallocAlign32(void **ptr, size_t size, aclrtMemMallocPolicy policy) {
    if (thread_local_config.is_interesting_region_) {
        return TorchMemorySaver::instance().malloc(
            ptr, CANNUtils::cann_ctx_get_device(), size, thread_local_config.current_tag_, thread_local_config.enable_cpu_backup_);
    } else {
...
void *tms_torch_malloc(ssize_t size, int device, aclrtStream stream) {
    ...
    TorchMemorySaver::instance().malloc(
        &ptr, CANNUtils::cann_device_get(device), size, thread_local_config.current_tag_, thread_local_config.enable_cpu_backup_);
    return ptr;
}
